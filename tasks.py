...
SOURCE_DIR = ROOT_DIR.joinpath("kaggle_brane")
...
def build(c):
    ...
    """
    print("building the brane package...")
    c.run("brane unpublish -f kaggle 1.0.0")
    c.run("brane remove -f kaggle")
    c.run("brane build -c {} container.yml".format(ROOT_DIR))
    c.run("brane push kaggle 1.0.0")
